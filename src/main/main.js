...
const { buildConfig, buildTestConfig, resolverBypassIps } = require('./configBuilder');
...
async function doConnect(serverId, opts = {}) {
  ...
  if (stale()) {
    // doDisconnect() ran its release() before this call wrote the state file, so
    // the override it just applied would outlive the tunnel it points at — the
    // machine would be left unable to resolve anything. Undo it here.
...
process.on('exit', () => {
  ...
  try { if (tun) tun.cleanupSync(); } catch {}   // the instance doConnect() started (either backend)
