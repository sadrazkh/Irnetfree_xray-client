...
4. بعد از ریبوت، قطعی برق یا ری‌استارت سرویس، روتر خودش به **همان اتصال قبلی** وصل می‌شود (سرور، زنجیره یا روتینگ ویژه):
   «اتصال خودکار» روی روتر **پیش‌فرض روشن** است و تا وصل نشده دست برنمی‌دارد — پنج دقیقهٔ اول هر ۱۵ ثانیه، بعد هر
   دقیقه (WAN، مودم و ساعت روتر بعد از بوت دیر بالا می‌آیند). وصل یا قطع کردن دستی تلاش‌ها را تمام می‌کند. از تنظیمات
   خاموش می‌شود.
5. اگر وسط کار `sing-box` یا هستهٔ `xray` بیفتد (کمبود حافظه، کرش)، سرویس خودش گیت‌وی را دوباره می‌سازد و تا وقتی قطع
   نکرده‌ای تلاش را ادامه می‌دهد (فاصله تا یک دقیقه). گیت‌ویی که بالا نیاید «اتصال ناموفق» است، نه «وصل، فقط پروکسی».
